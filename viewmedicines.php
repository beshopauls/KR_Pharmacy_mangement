...
try {
  ...
  $data = $db->query("SELECT MedId,MedName,MedDosage,MedPrice,ExpDate,ManufactureDate,QuantityBuy,QuantityDay,Type,Manufacturer
                       FROM Medicines
                            INNER JOIN categorymed
                                  ON categorymed.Id = medicines.CategoryMed
                            INNER JOIN manufacturer
                                  ON manufacturer.Id = medicines.MedManufacturer 
                      GROUP BY MedId")->fetchAll();
    ...
    <table class="table medicines">
      <thead>
        <tr>
          ...
          <th>Price</th>
          <th>Expir Date</th>
          <th class="active">Manifacture Date</th>
          <th class="active">Quantity Buy</th>
          <th class="active">Quantity Day</th>
          <th>Category</th>
          <th>Manifa-cturer</th>
        </tr>
      </thead>
      <tbody>
        <!-- MedId,MedName,MedDosage,MedPrice,ExpDate,ManufactureDate,QuantityBuy,QuantityDay,Type,Manufacturer -->
        <?php foreach ($data as $user) {
          ...
          echo  '<td class="active">' . $user['QuantityDay'] . "</td>";
          echo  "<td>" . $user['Type'] . "</td>";
          echo  "<td>" . $user['Manufacturer'] . "</td>" . "</tr>";
        } ?>
      </tbody>
    </table>
  </div>
